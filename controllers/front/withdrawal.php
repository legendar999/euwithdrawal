...
class EuwithdrawalWithdrawalModuleFrontController extends ModuleFrontController
{
    ...
    public function initContent()
    {
        ...
        if (Tools::isSubmit('euw_back')) {
            ...
            $this->collectInput();
        } elseif ('confirm' === $step && Tools::isSubmit('euw_submit')) {
            $rendered = $this->processConfirm();
        } elseif ('review' === $step && Tools::isSubmit('euw_continue')) {
    ...
    protected function resolveScope($id_order)
    {
        ...
        if (!$allow || EuWithdrawalRequest::SCOPE_ITEMS !== $scope) {
            return [EuWithdrawalRequest::SCOPE_ORDER, []];
        }
        ...
        return [EuWithdrawalRequest::SCOPE_ITEMS, $selected];
    }
    ...
    protected function notify($request, $order, $items)
    {
        ...
        $base_vars = [
            ...
            '{scope}' => (EuWithdrawalRequest::SCOPE_ITEMS === $request->scope)
        ...
        if (Configuration::get('EUWITHDRAWAL_NOTIFY_MERCHANT') && $merchant_email) {
            ...
            $merchant_vars = array_merge($base_vars, [
    ...
    {
        if (EuWithdrawalRequest::SCOPE_ITEMS !== $scope || empty($items)) {
